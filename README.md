...
- Create a copy of `home_expenses/settings/local.py.example`:
  `cp home_expenses/settings/local.py.example home_expenses/settings/local.py`
...
- Open a new command line window and go to the project's directory.
- Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv home_expenses` or `python -m venv home_expenses-venv`
...
- Make sure the virtualenv is activated `workon home_expenses` or `source home_expenses-venv/bin/activate`
...
- `workon home_expenses` or `source home_expenses-venv/bin/activate` depending on if you are using virtualenvwrapper or just virtualenv.
